...
```{r message=FALSE, warning=FALSE}
datasets <- datasets %>%
  left_join(m49::m49_full %>% mutate(country_or_area = stringr::str_replace(country_or_area, "Turkey", "Türkiye")) %>% select(country_or_area, ldc, lldc, sids, developed, developing, region_name), by = c("country" = "country_or_area")) %>%
